...
void expe_onece(int i,int argc,char * argv[],std::ofstream *fs,std::ofstream *fs2,size_t data_size){
 ...
 //let_period=atol(argv[6])/EXPERIENC_NUM;
  //let_period = let_period*(i+1);
  ...
  std::thread thread_be([&]() {
       if(scheduler.compare("let")==0)
      {
       ...
       std::cout << "Thread with id=" << std::this_thread::get_id() << " is going to call executor_be.executor_spin_period() ..." << std::endl;
  ...
  *fs << "Data size: " << data_size <<"byte\n";
  *fs << "CTime: " << rt_busyloop_us.count() <<  " us "  << be_busyloop_us.count() << "us\n";
  *fs <<" rt LET period: " << rt_let_period<< "ns\n" ;
  *fs <<" be LET period: " << be_let_period<< "ns\n" ;
  ...
  *fs2 << rt_let_period << std::endl;
  *fs2 << be_let_period << std::endl;
  *fs2 << data_size << std::endl;
  *fs2 << data_size << std::endl;
...
int main(int argc, char * argv[])
{
  ...
  fs.open("exp3Result.txt");
  fs2.open("exp3Result2.txt");
  int size = 0;
  size = atoi(argv[10]);
  ...
  for(int i = 0; i < EXPERIENC_NUM; i++){
    ...
    expe_onece(i,argc, argv, &fs,&fs2,data_size_);
     data_.resize(data_size_ +1000 ,rand() % 26 + 'a');
