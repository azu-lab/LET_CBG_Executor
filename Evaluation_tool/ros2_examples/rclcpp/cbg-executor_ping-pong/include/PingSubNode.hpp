...
public:
  ...
  void print_jitter();
  void outputjitter(std::vector<int64_t> jitters);
  void save_statistics(std::ofstream *fs);
  void save_statistics2(std::ofstream *fs);
...
private:
  ...
  std::vector<std::chrono::system_clock::duration> jitters{};
  std::vector<std::chrono::system_clock::duration> latencys{};
