...
public:
  ...
  RCLCPP_PUBLIC
  std::vector<int64_t>
  spin(const uint64_t period);
   RCLCPP_PUBLIC
  void
  spin_some(std::chrono::nanoseconds max_duration);
  RCLCPP_PUBLIC
  void
  executor_spin_one_period(const uint64_t period,std::string thread_name1);
  RCLCPP_PUBLIC
  void
  executor_spin_period(const uint64_t period,std::string thread_name1);
